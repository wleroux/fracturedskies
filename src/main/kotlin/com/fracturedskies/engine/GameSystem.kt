...
abstract class GameSystem(coroutineContext: CoroutineContext = DefaultDispatcher) {
  companion object {
    fun register(gameSystem: GameSystem) {
      MessageBus.register(gameSystem.messageChannel)
    }
    fun unregister(gameSystem: GameSystem) {
      MessageBus.unregister(gameSystem.messageChannel)
    }
  }
