...
class BasicTreePopulator(private val random: Random): Populator {
  ...
  private fun placeTree(blocks: ObjectMutableSpace<Block>, pos: Vector3i) {
    if (blocks[pos].type != DIRT && blocks[pos].type != GRASS) return
    ...
    (0 until height).forEach { y ->
      blockPlacements[Vector3i(0, y, 0)] = WOOD
    }
    blockPlacements[Vector3i(0, height, 0)] = LEAVE
    blockPlacements[Vector3i(0, height + 1, 0)] = LEAVE
    (-1..1).forEach { dx ->
      (-1..1).forEach { dz ->
        (leaveStartHeight..height).forEach { dy ->
          if (dx.absoluteValue == 1 && dz.absoluteValue == 1) {
            if (random.nextBoolean()) {
              blockPlacements.putIfAbsent(Vector3i(dx, dy, dz), LEAVE)
            }
          } else {
            blockPlacements.putIfAbsent(Vector3i(dx, dy, dz), LEAVE)
          }
    ...
    // Try placing tree
    if (!blockPlacements.keys.map { pos + Vector3i.AXIS_Y + it }.all { blocks.has(it) && blocks[it].type == AIR }) return
  ...
  private fun highestBlock(blocks: ObjectMutableSpace<Block>, x: Int, z: Int): Vector3i? {
    val y = (0 until blocks.height).reversed().firstOrNull { y -> blocks[x, y, z].type != AIR }
