package com.fracturedskies.render.components.input
...
import com.fracturedskies.render.components.TextRenderer
...
class Input(attributes: Context) : AbstractComponent<Input.InputState>(attributes, InputState(attributes[INITIAL_VALUE] ?: "")) {
  ...
  override fun willMount() {
    material = Material(StandardShaderProgram(), Context(
            StandardShaderProgram.ALBEDO to TextureArray("input_default.png", loadByteBuffer("com/fracturedskies/render/components/input/input_default.png", this.javaClass.classLoader), 4, 4, 9)
    ))
  }
  ...
  private fun doOnKey(event: com.fracturedskies.render.events.Key) {
  ...
  override val handler = EventHandlers(
          ...
          on(com.fracturedskies.render.events.Key::class) { doOnKey(it) },
