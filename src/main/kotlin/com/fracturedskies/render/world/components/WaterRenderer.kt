...
class WaterRenderer(props: MultiTypeMap) : Component<Unit>(props, Unit) {
  ...
  companion object {
    fun Node.Builder<*>.water(worldState: RenderWorld, sliceHeight: Int, additionalProps: MultiTypeMap = MultiTypeMap()) {
  ...
  override fun glRender(bounds: Bounds) {
    ...
    chunks.forEach { chunkIndex ->
      ...
      val mesh = if (shouldChunkUpdate) {
        ...
        prevMesh?.close()
        val newMesh = generateWaterMesh(worldState.blocks, false,
      ...
      val sliceMesh = if (shouldChunkUpdate) {
        ...
        val nextSliceMesh = if (sliceHeight in (chunkPos.y * CHUNK_Y_SIZE until (chunkPos.y + 1) * CHUNK_Y_SIZE)) {
          generateWaterMesh(worldState.blocks, true,
              chunkPos.x * CHUNK_X_SIZE until (chunkPos.x + 1) * CHUNK_X_SIZE,
