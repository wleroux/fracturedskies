...
interface Condition {
  fun matches(world: WorldState, colonist: Colonist, task: Task): Boolean
}

class AllOfCondition(private vararg val conditions: Condition): Condition {
  override fun matches(world: WorldState, colonist: Colonist, task: Task) = conditions.all { it.matches(world, colonist, task) }
...
class AnyOfCondition(private vararg val conditions: Condition): Condition {
  override fun matches(world: WorldState, colonist: Colonist, task: Task) = conditions.any { it.matches(world, colonist, task) }
  override fun toString() = "any of ${conditions.joinToString(prefix = "[", separator = ",", postfix = "]")}"
}

object NoneCondition: Condition {
  override fun matches(world: WorldState, colonist: Colonist, task: Task) = false
  override fun toString() = "none"
}

object AllCondition: Condition {
  override fun matches(world: WorldState, colonist: Colonist, task: Task) = true
  override fun toString() = "all"
}

object SingleAssigneeCondition: Condition {
  override fun matches(world: WorldState, colonist: Colonist, task: Task) =
