package com.fracturedskies.game.render.components.world

import com.fracturedskies.engine.collections.ObjectMap
import com.fracturedskies.engine.math.*
import com.fracturedskies.game.BlockType
import com.fracturedskies.game.render.shaders.Mesh
import com.fracturedskies.game.water.WaterSystem.Companion.MAX_WATER_LEVEL
import org.lwjgl.BufferUtils.*


fun generateWaterMesh(
    world: ObjectMap<com.fracturedskies.game.render.components.world.Block>,
    sliceMesh: Boolean,
    xRange: IntRange, yRange: IntRange, zRange: IntRange
): () -> Mesh {
  val quads = mutableListOf<com.fracturedskies.game.render.components.world.Quad>()
  for (y in yRange) {
    for (x in xRange) {
      for (z in zRange) {
        val pos = Vector3i(x, y, z)
        val block = world[pos]
        if (block.type !== BlockType.AIR) {
          continue
        }

        val curWaterLevel = block.waterLevel
        if (curWaterLevel == 0.toByte()) {
          continue
        }

        val xOffset = x.toFloat()
        val yOffset = y.toFloat()
        val zOffset = z.toFloat()
        val curWaterHeight = waterHeight(curWaterLevel)

        // north
        val northPos =  pos + Vector3i.AXIS_NEG_Z
        if (!sliceMesh && isEmpty(world, northPos)) {
          val adjWaterLevel = if (!world.has(northPos)) 0 else world[northPos].waterLevel
          val adjWaterHeight = waterHeight(adjWaterLevel)

          if (adjWaterHeight < curWaterHeight) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_NEG_Y) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_NEG_Y) }

            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset, 1f, 0f,
                yOffset + curWaterHeight, 0f, adjWaterHeight - curWaterHeight,
                zOffset, 0f, 0f,
                0f, 0f, -1f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }


        // up
        val upPos = pos + Vector3i.AXIS_Y
        if (sliceMesh || isEmpty(world, upPos) || curWaterLevel != MAX_WATER_LEVEL) {
          val adjWaterLevel = if (world.has(upPos)) world[upPos].waterLevel else 0
          val drawMesh = when (sliceMesh) {
            true -> curWaterLevel == MAX_WATER_LEVEL
            false -> curWaterLevel != MAX_WATER_LEVEL || adjWaterLevel == 0.toByte()
          }
          if (drawMesh) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_NEG_Z) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_NEG_Z) }
            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset, 1f, 0f,
                yOffset + curWaterHeight, 0f, 0f,
                zOffset + 1f, 0f, -1f,
                0f, 1f, 0f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }

        // west
        val westPos = pos + Vector3i.AXIS_NEG_X
        if (!sliceMesh && isEmpty(world, westPos)) {
          val adjWaterLevel = if (world.has(westPos)) world[westPos].waterLevel else 0
          val adjWaterHeight = waterHeight(adjWaterLevel)

          if (adjWaterHeight < curWaterHeight) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_NEG_Z, Vector3i.AXIS_NEG_Y) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_NEG_Z, Vector3i.AXIS_NEG_Y) }
            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset, 0f, 0f,
                yOffset + curWaterHeight, 0f, adjWaterHeight - curWaterHeight,
                zOffset + 1f, -1f, 0f,
                -1f, 0f, 0f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }

        // east
        val eastPos = pos + Vector3i.AXIS_X
        if (!sliceMesh && isEmpty(world, eastPos)) {
          val adjWaterLevel = if (world.has(eastPos)) world[eastPos].waterLevel else 0
          val adjWaterHeight = waterHeight(adjWaterLevel)

          if (adjWaterHeight < curWaterHeight) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_Z, Vector3i.AXIS_NEG_Y) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_Z, Vector3i.AXIS_NEG_Y) }
            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset + 1f, 0f, 0f,
                yOffset + curWaterHeight, 0f, adjWaterHeight - curWaterHeight,
                zOffset, 1f, 0f,
                1f, 0f, 0f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }

        // down
        val downPos = pos + Vector3i.AXIS_NEG_Y
        if (!sliceMesh && isEmpty(world, downPos)) {
          val adjWaterLevel = if (world.has(downPos)) world[downPos].waterLevel else 0
          if (adjWaterLevel != MAX_WATER_LEVEL) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_Z) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_X, Vector3i.AXIS_Z) }

            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset, 1f, 0f,
                yOffset, 0f, 0f,
                zOffset, 0f, 1f,
                0f, -1f, 0f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }

        // south
        val southPos = pos + Vector3i.AXIS_Z
        if (!sliceMesh && isEmpty(world, southPos)) {
          val adjWaterLevel = if (world.has(southPos)) world[southPos].waterLevel else 0
          val adjWaterHeight = waterHeight(adjWaterLevel)

          if (adjWaterHeight < curWaterHeight) {
            val skyLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.skyLightLevel(world, pos, Vector3i.AXIS_NEG_X, Vector3i.AXIS_NEG_Y) }
            val blockLightLevels = com.fracturedskies.game.render.components.world.VertexCorner.values().associate { it to it.blockLightLevel(world, pos, Vector3i.AXIS_NEG_X, Vector3i.AXIS_NEG_Y) }
            quads.add(com.fracturedskies.game.render.components.world.Quad(
                xOffset + 1f, -1f, 0f,
                yOffset + curWaterHeight, 0f, adjWaterHeight - curWaterHeight,
                zOffset + 1f, 0f, 0f,
                0f, 0f, 1f,
                skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, skyLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_LEFT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.TOP_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_RIGHT] ?: 0f, blockLightLevels[com.fracturedskies.game.render.components.world.VertexCorner.BOTTOM_LEFT] ?: 0f,
                Color4.WATER,
                0f, 0f, 0f, 0f
            ))
          }
        }
      }
    }
  }

  val attributeSize = com.fracturedskies.game.render.components.world.Quad.Attributes.fold(0, { acc, attr -> acc + attr.elements * attr.elementSize}) / java.lang.Float.BYTES
  val verticesBuffer = createFloatBuffer(attributeSize * 4 * quads.size)
  val indicesBuffer = createIntBuffer(6 * quads.size)

  var vertexCount = 0
  quads.forEach {quad ->
    verticesBuffer.put(quad.vertices())
    indicesBuffer.put(quad.indices(vertexCount))
    vertexCount += 4
  }

  verticesBuffer.flip()
  val vertices = FloatArray(verticesBuffer.remaining())
  verticesBuffer.get(vertices)

  indicesBuffer.flip()
  val indices = IntArray(indicesBuffer.remaining())
  indicesBuffer.get(indices)

  return { Mesh(vertices, indices, com.fracturedskies.game.render.components.world.Quad.Attributes) }
}

private fun waterHeight(waterLevel: Byte): Float {
  return map(waterLevel.toFloat(), 0f..MAX_WATER_LEVEL.toFloat(), 0f..1f)
}

private fun isEmpty(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i): Boolean {
  return if (!world.has(pos)) true else world[pos].type === BlockType.AIR
}
