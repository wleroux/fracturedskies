...
enum class Occlusion(private val offset: (Vector3i, Vector3i, Vector3i) -> Vector3i) {
  ...
  fun gathersLight(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i, u: Vector3i, v: Vector3i): Boolean {
    val target = offset(pos, u, v)
    return world.has(target) && !world[target].type.opaque
  }
  fun opaque(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i, u: Vector3i, v: Vector3i): Boolean {
    val target = offset(pos, u, v)
    return if (world.has(target)) world[target].type.opaque else false
  }
  fun skyLight(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i, u: Vector3i, v: Vector3i): Int {
    val target = offset(pos, u, v)
    return if (world.has(target)) world[target].skyLight else 0
  }
  fun blockLight(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i, u: Vector3i, v: Vector3i): Int {
    val target = offset(pos, u, v)
    return if (world.has(target)) world[target].blockLight else 0
  }

  companion object {
    fun of(world: ObjectMap<com.fracturedskies.game.render.components.world.Block>, pos: Vector3i, u: Vector3i, v: Vector3i): EnumSet<Occlusion> {
